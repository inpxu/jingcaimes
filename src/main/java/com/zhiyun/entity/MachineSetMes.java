...
public class MachineSetMes extends BaseEntity<Long> {
	...
	// ~~~~实体属性
	// 设备编号
	@Pattern(regexp="[\\s\\S]{0,40}", message="设备编号字段过长")
	private String macNo;
	// 设备名称
	@Pattern(regexp="[\\s\\S]{0,40}", message="设备名称字段过长")
	...
	private Long orgId;
	// 设备类型
	@Pattern(regexp="[\\s\\S]{0,20}", message="设备类型字段过长")
	private String macType;
	// 工艺ID
	@Max(value=9223372036854775807L,message="工艺ID字段过长")
	private Long crafworkId;
	// 设备责任人
	@Pattern(regexp="[\\s\\S]{0,30}", message="设备责任人字段过长")
	...
	// 品牌
	@Pattern(regexp="[\\s\\S]{0,40}", message="品牌字段过长")
	private String brand;
	// 参数规格
	@Pattern(regexp="[\\s\\S]{0,20}", message="参数规格字段过长")
	private String spec;
	// 保养间隔时数
	@Max(value=99999999999L,message="保养间隔时数字段过长")
	private Integer fixHour;
	// 设备状态
	@Pattern(regexp="[\\s\\S]{0,10}", message="设备状态字段过长")
	private String macStatus;
	// 条形码
	@Pattern(regexp="[\\s\\S]{0,30}", message="条形码字段过长")
	private String barcode;
	// 二维码
	@Pattern(regexp="[\\s\\S]{0,30}", message="二维码字段过长")
